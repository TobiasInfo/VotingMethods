...
func TestCopelandSWF(t *testing.T) {
	...
	// Expected results
	expectedCount := Count{
		1: 1,  // 1 wins over 2, ties with 3
		2: -1, // 2 loses to 1, loses to 3
		3: 0,  // 3 ties with 1, wins over 2
	}

	count, err := CopelandSWF(profile)
	if err != nil {
		t.Fatalf("Error in CopelandSWF: %v", err)
	}

	for alt, score := range expectedCount {
		if count[alt] != score {
			t.Errorf("Expected score for alternative %d: %d, got: %d", alt, score, count[alt])
		}
	}
}
...
func TestCopelandSCF(t *testing.T) {
	...
	// Expected winner is alternative 1 based on Copeland rule
	expectedWinner := []Alternative{1}

	winner, err := CopelandSCF(profile)
	if err != nil {
		t.Fatalf("Error in CopelandSCF: %v", err)
	}

	if len(winner) != len(expectedWinner) || winner[0] != expectedWinner[0] {
		t.Errorf("Expected winner: %v, got: %v", expectedWinner, winner)
	}
}
